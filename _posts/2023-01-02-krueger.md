...
* [Latest Research From David Krueger's Lab](#latest-research-from-david-kruegers-lab)
    * [Metadata Archeology](#metadata-archeology)
    * [Understanding The Learning Process And Generalization](#understanding-the-learning-process-and-generalization)
...
* [Safety-Performance Trade-Offs](#safety-performance-trade-offs)
    ...
    * [More Testing, Humans In The Loop And Saving Throws From Hacks](#most-of-the-risk-comes-from-safety-performance-trade-offs-in-developement-and-deployment)
    ...
    * [Training Models That Do Not Reason About Humans: Another Safety-Performance Tradef-off](#trainings-models-that-do-not-reason-about-humans-another-safety-performance-tradef-off)
...
* [Recursive Self Improvement, Bitter Lesson And Alignment](#recursive-self-improvement-bitter-lesson-and-alignment)
    * [Recursive Self Improvement By Cobbling Together Different Pieces](#recursive-self-improvement-by-cobbling-together-different-pieces)
...
* [David's Approach To Doing Research](#david)
...
* [Times Past And Ahead](#times-past-and-ahead)
...
**Michaël**:
In Cambridge, your students call you "The Krug" or "DK", short for David Krueger. And it's a pleasure to have the man himself on the show. Thanks David for joining us.

**David**:
Wow. I've never heard them call me that to my face.

**Michaël**:
Soon on the street people will call you DK. You're also the supervisor of Ethan Caballero... so you're the last supervisor—we have had [Irina](https://theinsideview.ai/irina), [Blake Richards](https://theinsideview.ai/blake), and now you're the third one. The last boss. [Last time](https://theinsideview.ai/ethan2) with Ethan, we discussed the performance on all downstream tasks and learning curves and I thought it would make sense to just start with learning curves. Why do they matter? And you have published some papers on this. What do we mean by learning curves? What kind of research have you done?

# Latest Research From David Krueger's Lab

**David**:
There's the paper [Broken Neural Scaling Laws](https://arxiv.org/abs/2210.14891) that you're referencing and then there are two other papers, but I noticed after the fact that these were all of a same or similar theme. I didn't really go out aiming to do this kind of research. But the idea is learning curves are just telling you how your performance changes over time as you learn. Or you can say scaling curves is a generalization of that where you also look at things like the size of the model or the amount of data you have. So data, compute, and parameters. People usually look at learning like training and validation curves, which is just the aggregate performance over the entire training on the validation dataset throughout training.

## Metadata Archeology

**David**:
Recently people have started to pay more attention to evaluations but are usually doing that once the model has been trained. And the idea here is just that we want to be doing a lot of evaluation of the model and tracking a lot of different things while it's learning. And we really want to use that as a way to study the dynamics of learning and how learning works. We can do that by looking at different subsets of the data, trying to discover which examples belong to different subsets or have different metadata. That's one of the other papers, ['Metadata Archeology'](https://metadata-archaeology.github.io/), where we have some examples that we assume we know the metadata for. So this might be something like who labeled the data or where was it collected. Information about how it was collected in general. There could also be things about if the data has been corrupted or mislabeled, things like this. That's the  metadata that we use in that paper.

**David**:
And then you can look at how the model learns those examples for which you know the metadata and use that to try and infer the metadata for other examples where you don't know the metadata. That's in a sense like a supervised learning approach because we have these ground-truth metadata for some of the examples. But you can also imagine doing this in an unsupervised way. So just looking per example learning curves and then trying to cluster those. And of course you don't just have to look at the performance, you can also look at the predictions or other  behavior of the model on that particular example throughout training and see how those evolved.

**David**:
So there's another paper that came out around the same time as the metadata archeology paper that was doing this looking at, I forget what they called this, something like example profiles or something. They would look at a single example and see what the predictions on that example were and how that changed over time. That was a really interesting thing to look at as well. I think the general direction here is just to study the learning and generalization behavior, try and get a better understanding through kind of scientific empirical approach of how the stuff works. And in particular do this by doing a more finegrained analysis where we pay attention to different subsets of the data, different types of examples, and try and pick up on important trends that occur during learning. You might find at some point that the model is paying attention to some spurious feature like the background and then it stops paying attention to it later in training. Or the opposite could happen. You could also find, especially if you're using synthetic data, which is the third project that I haven't really talked about, is more in that line of work. You can think about what are the mechanisms that the model is paying attention to or what is the algorithm that it's implementing.

**David**:
So you know, you might have some examples that are hard to solve without doing some more complex cognition that looks more like planning or reasoning. And then you could see how does the model treat those examples. You always have to watch out for memorization as a potential confounder here. Just because the model is fitting an example, doesn't mean that it has learned something general about that class of example.

## Understanding The Learning Process And Generalization

**Michaël**:
Is the basic idea to see how well a model generalizes? If it's just memorizing the background noise or memorizing the data? Is that how it is related to alignment or safety in general, making sure models have a good generalization overall?
...
**David**:
I'm not sure about that. Good question. How would this actually show up? I guess what I'm imagining is something more where you have some examples that you use to evaluate deception in some way or I haven't thought about the details here much to be honest.
...
**David**:
Yeah, so far it's just entirely abstract.
...
**David**:
Yeah, it is at the Machine Learning Safety [workshop](https://neurips2022.mlsafety.org/) and a few other workshops at NeurIPS.
...
**David**:
If it does the same thing every time it's in a certain state, that makes it look more goal directed. And of course if you have some more prior knowledge about what are reasonable goals, then you can make this work even in palm MDP and so on and so forth.
...
**Michaël**:
It both helps the AI better understand the instructions but at the same time maybe become more agentic and dangerous.
...
**David**:
No. If it's the right distribution, then you're good. But my point is the distribution matters. So it's not just the coverage, but it's the actual distribution.
...
**Michaël**:
I won't go deep into all the decision theories because I haven't read all of them. Do you remember why were we talking about agents?
...
**David**:
That's a good question. I guess it depends on how longtermist you are, but I think we can say definitely a lot because the amount of harm that we're talking about is at least killing everyone alive right now. And then if you value future people to some extent, then that starts to grow more.
...
**Michaël**:
Do you agree with the [slow takeoff](https://www.lesswrong.com/tag/ai-takeoff) scenario?
...
**Michaël**:
So some kind of centralized system where people can just pre-commit to some action if millions of people are boycotting some company.
...
**David**:
I'm not sure I've understood, but it sounds like a more specific version of what I'm saying basically.
...
**Michaël**:
So you have both the, let's say, flexibility of choosing what research you want to do, but at the same time you're trying to go for the rigor of the original academic discipline?
...
**Michaël**:
So you're saying if you're not interacting with the entire research committee as a whole, you may be in a situation where you end up doing the wrong things because you don't have enough peer review, people looking at the problem?
...
**David**:
I think so. I think something like this is used by [Anthropic](https://www.anthropic.com/) and [OpenAI](https://openai.com/), maybe sometimes by Google. I think Google is a little bit more just on the imitation learning side for the fine-tuning of alignment.
...
**Michaël**:
We've been bouncing around this notion of alignment or getting models to do what we want. Do you have any more precise definition? Because I've tried to talk to Mila people about it and it seemed like even if you were there for a few years, people didn't know exactly what alignment was.
...
**Michaël**:
Most people I was talking to at Mila would just say, of course we want models that do what we want because models will do what we want but if we design it properly, it's not an actual problem. And I think that's one crux is people are mostly optimistic about the thing doing what we want by default.
...
**David**:
I don't want to manufacture outrage. I think outrage will happen more naturally maybe. I think it's more just getting people to understand.
...
**Michaël**:
It is pretty easy to understand and elegant.
...
**Michaël**:
You mean your students? If you talk to other professors in Cambridge in UK or in the US, maybe they would have similar rationale.
...
**David**:
Yeah, I think that's bullshit. This is building the legend, right?

## Timeline? Pretty Soon...

**Michaël**:
I haven't asked. What are your timelines?
...
**Michaël**:
Okay. I think that's a good ending.
