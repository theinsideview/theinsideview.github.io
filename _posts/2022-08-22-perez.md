...
* [The Inverse Scaling Prize](#the-inverse-scaling-prize)
    ...
    * [Defining Misalignment](#defining-misalignment)
    * [Required Compute For Submissions](#required-compute-for-submissions)
    * [Why Inner Alignment is Not Out of Distribution Generalization](#why-inner-alignment-is-not-out-of-distribution-generalization)
    * [Can Inverse Scaling Detect Deception](#can-inverse-scaling-detect-deception)
...
**Michaël**: When talking about objectives, are you pointing out losses from deep learning models or something else?
...
**Michaël**: Just to not lose the people that are starting to watch this video or listen to this podcast and do actually care about aligning super intelligent models. What do you have to say to them to prove that you're actually doing long-term alignment work?
<a href="#outline">⬆</a>
...
**Michaël**: It mostly allows/laws [inaudible] for validation or test where you're trying to see if the model, will choose different actions, based on your test data.
...
**Ethan**: Well, it could be smaller. Yeah. I guess everything has scaling laws. So if you're working on small models here, to some extent, you're working on large files.
...
**Ethan**: This one was a zero-shot example. It's basically a question answering task or the question includes first a statement about what the person asking the question believes.
...
## Required Compute For Submissions
...
**Michaël**: Yeah, because with the API, you can also check for different sizes. There are different WG [inaudible] models.
...
## Why Inner Alignment is Not Out of Distribution Generalization
...
**Michaël**: I won't go into this evolution analogy, because I've been talking a lot about this in the podcast. And I always end up disagreeing with people about whether there's an actual optimization process happening or not. I think when people talk about inner misalignment, they think about a different optimizing process. If there was another objective being optimized on top of just minimizing some kind of test loss [inaudible], there was another optimizer inside the weights of your neural network. And I think that's why it's kind of harder to find test cases for these because there has not been any example of this implementing code, as of right now, I believe.
...
## Can Inverse Scaling Detect Deception
...
**Michaël**: And this is kind of different from what you did. So instead of having one bit by one bit of information, you get an actual sentence from a human. So let's say the human says 10 words. That's maybe 20 tokens. Do you know how it compares to InstructGPT in terms of the amount of text or bits of information you give the model to [inaudible]? If I have 10 bits of comparison in InstructGPT, how many sentences do I need to say?
...
**Michaël**: Originally, red teaming comes from, I don't know if it's like Capture the Flag thing or hiker [inaudible] competitions where you have a team that tries to fight another team.
...
**Michaël**: There's something in my license that says, "If you're using this for commercial applications, then..." Yeah. I think one of the other things you mentioned in the paper is on top of prompt engineering, you also do, [inaudible] RL, reinforcement learning methods. Can you maybe elaborate on those?
